...
function drawHero(
  ...
  const pose = weaponPose(
    action,
    state.stats.timeMs,
    heldTwoHanded(state.players[0].equipment.weapon.defId),
  );
...
function drawDeadHero(
  ...
  const cy = Math.round(state.players[0].pos.y - camera.y + 5); // the ground line
...
export function drawLevelUpBurn(
  ...
  const x = Math.round(state.players[0].pos.x - camera.x);
  const y = Math.round(state.players[0].pos.y - camera.y - state.players[0].z);
  ...
  const power = levelUpIntensity(state.players[0].level);
