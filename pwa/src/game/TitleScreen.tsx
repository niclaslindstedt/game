...
export function TitleScreen({
  ...
  // The hidden developer gesture, in two movements: sixteen quick taps on the
  // title sky's sun ARM it — the first ten in total silence, so the sky only
  // starts answering on the eleventh — then a CLICK RACE (a press every 250 ms,
  // five banked seconds of it) swells the star until it lets go (the counting,
  // the beat and the build-up live in TitleBackdrop / use-sun-charge.ts /
  ...
  // (and the menu it opens) folds out of the bundle. The hook checks the same
  // flag itself, so the sun stays inert there however this prop is passed.
