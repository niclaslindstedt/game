...
// screen. The port is a direct page↔session link. The large, frequent SERVER →
// CLIENT snapshots are transferred by the server; the tiny CLIENT → SERVER
// input/command frames are copied here. A port handed into the page by a shell
// may be a proxy on which a transfer list throws instead of sending — leaving
// the session visibly alive while every keyboard and pointer input disappears.
  ...
  return {
    send(frame) {
      ...
      try {
        ...
        // save one small copy, but a shell's proxied port may not carry a
        // transfer list at all. Input frames are tiny; snapshots remain
        // zero-copy in the other direction.
