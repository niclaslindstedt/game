...
| `simulate-run`     | Measuring ACTUAL balance by running the real engine headlessly (`scripts/simulate-run.mjs`; engine side `src/sim/simulate.ts`): whole levels or whole campaigns easy → JESUS with the autopilot, auto-equip, and loadout carry — reporting hero/mob hp and damage, drops, weapon swaps, deaths, and XP withheld by the per-map caps.          |
