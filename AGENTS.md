...
**Steam builds only.** PRs 1, 2, 1.5, 1.75 and 2.5 of the eight in
`docs/multiplayer-plan.md` have landed: a session server, the wire, the fifth
bridge, two transports, a challenge handshake, remote SPECTATORS, chat,
`/players N`, a closed list of 69 named commands covering everything the app DOES
to a run — and, from PR 2.5, the three doors a player walks through (HOST GAME,
the server browser, JOIN BY ADDRESS), the in-run chat, the live session panel and
the invite launch arguments.

**IT IS REACHABLE NOW, AND WHAT IS MISSING IS A SECOND HERO.** Every joiner is a
SPECTATOR: they see the run, stand on the roster, and may talk — `state.player`
is still one hero, and seating a second is PR 3. Two consequences worth knowing
before touching anything here. A spectator plays on a THROWAWAY character
(`spectatorCharacter`), so nothing they watch can be banked onto their roster;
and their run commands travel but are NOT applied locally
(`setCommandSink(…, { optimistic: false })`), because the session refuses them
