...
- **Levels are compiled from YAML**, the same way. `website/scripts/levels/<id>.yaml`
  ...
  level change with `node scripts/update-level-snapshot.mjs`. Read a map's design
  with `make map LEVEL=<id>` (`website/scripts/map-preview.mjs`).
...
| `level-design`     | Adding a new level/mission — the YAML level format (`website/scripts/levels/<id>.yaml`, compiled by `make levels`), the annotated map renderer (`website/scripts/map-preview.mjs`), the design-zone systems (safe/quiet zones, tempo, chests, merchant spawns), campaign registration and unlock order, spawn/wave/pack budgets, the cumulative loot-pool rule, XP/arrow-cap pacing wiring, and the checker + test battery a new map must pass. |
| `map-improvement`  | Improving an EXISTING map's design and FEEL — the render → evaluate → improve loop. Confirms the intended feel with the user FIRST (the YAML descriptions may be wrong), rewrites the description, then reads the annotated map + played heatmap (dwell/mob-density/coverage), retunes the YAML, re-renders, and presents before/after maps for sign-off before shipping.                                                                       |
