...
| `leveling-balance` | Tuning how fast the hero levels — the XP curve, kills-per-level pacing, the level cap, the onboarding ramp — via the kills-per-level model and the calculator (`scripts/leveling-curve.mjs`), then a bot run to measure the real kill rate.                                                                                                   |
