...
- **Levels are compiled from YAML**, the same way. `website/scripts/levels/<id>.yaml`
  ...
  (`website/scripts/map-layout.mjs` — the clean high-res blueprint: grid, walls,
  path, every spawn/encounter with hp/level), and how it plays with `make map
  LEVEL=<id>` (`website/scripts/map-preview.mjs` — design/`--actual`/`--heatmap`).
