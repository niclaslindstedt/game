...
//! to lose:
//!
//!  1. **One stable origin.** The player's whole roster lives in
//!     `localStorage`, which is keyed by origin. `game://` is a constant, so
//!     saves survive every update. A `file://` page (or a server on an
//!     ephemeral port) would hand the player a different origin — and an empty
...
//! normalized LEXICALLY — segments resolved, `..` refusing to climb past the
//! root — and the joined result is then checked to still start with the root.
//! Either half alone has a known hole (a lexical pass alone is defeated by a
//! symlink inside the webroot; a prefix check alone is defeated by a `..` that
//! the filesystem resolves before the comparison), and the pair is what the
//! TypeScript peer does with `resolve` + `startsWith`.
...
///
/// Every segment is walked: `.` is dropped, `..` pops (and pops NOTHING at the
/// top, so a path that tries to climb out arrives at the root rather than above
/// it), and a Windows prefix or a root component is refused outright — `game://
/// /C:/…` must not become a drive letter.
