...
> **This tree is at phase 2 of four.** It runs the whole game, offline, in a
> real window that remembers itself, and it carries the Steam seams: cloud save,
> achievements, screenshots, and a package. It has **no mods, no multiplayer, no
> voice and no Steam overlay** — the first three are routed to a seam that logs
> which phase fills them in, and the fourth is not coming (see below). The plan,
> the phases and the open design questions are
...
**Nothing here ships yet** — `electron/` is still the release package and that
decision belongs to phase 4. What exists is the machinery, so that when the
decision comes it is a choice rather than a project:
