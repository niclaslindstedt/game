...
describe("weapon durability", () => {
  ...
  it("each attack spends one point of durability", () => {
    ...
    clearStage(state);
    addPunchingBag(state);
    state.players[0].equipment.weapon = weapon(50, "test_hammer", 10);
    run(state, idle, 200, (s) => s.stats.damageDealt > 0); // one swing
    expect(state.players[0].equipment.weapon.durability).toBe(9);
  });
  ...
  it("a broken weapon drops into the bag (not trashed) and the best bag weapon takes over", () => {
    ...
    clearStage(state);
    addPunchingBag(state);
    state.players[0].equipment.weapon = weapon(50, "test_wand", 1); // last swing
    state.players[0].inventory[0] = weapon(51, "test_pistol"); // ~17.5 dps
    state.players[0].inventory[1] = weapon(52, "test_hammer"); // ~53 dps — the pick
    ...
    const brokenWand = state.players[0].inventory.find((i) => i?.id === 50);
    expect(brokenWand?.durability).toBe(0);
    expect(state.players[0].inventory[0]?.id).toBe(51); // the pistol stays put
  });
  ...
  it("prefers a good bag weapon over the starting sidearm on a break", () => {
    ...
    addPunchingBag(state);
    state.players[0].equipment.weapon = weapon(50, "test_wand", 1); // last swing
    state.players[0].inventory[0] = weapon(52, "test_hammer"); // the only spare
    run(state, idle, 400, (s) =>
      s.events.some((e) => e.type === "weaponBroke"),
    );
  ...
  it("with no wieldable weapon in the bag a fresh sidearm is drawn", () => {
    ...
    clearStage(state);
    addPunchingBag(state);
    state.players[0].equipment.weapon = weapon(50, "test_wand", 1);
    run(state, idle, 400, (s) =>
      s.events.some((e) => e.type === "weaponBroke"),
    );
    ...
    expect(
      state.players[0].inventory.find((i) => i?.id === 50)?.durability,
    ).toBe(0);
  });
...
describe("same-weapon pickups refresh durability", () => {
  ...
  it("picking up the same weapon swaps it in and banks the worn copy", () => {
    ...
    clearStage(state);
    const full = weaponDef("test_hammer").durability;
    state.players[0].equipment.weapon = weapon(50, "test_hammer", 4);
    state.items = [
      {
        id: 1,
        kind: "equipment",
    ...
    expect(state.players[0].equipment.weapon.durability).toBe(full);
  ...
  it("with a full bag it still swaps in, dropping the worn copy on the ground", () => {
    ...
    for (let i = 0; i < state.players[0].inventory.length; i++) {
      state.players[0].inventory[i] = weapon(100 + i, "test_pistol");
    }
    state.items = [
      {
        id: 1,
    ...
    expect(state.players[0].equipment.weapon.id).toBe(60);
    expect(state.players[0].inventory.some((i) => i?.id === 50)).toBe(false);
    expect(
      state.items.some(
...
describe("repair kits — banking", () => {
  it("stash into the dock on pickup (stacking) rather than firing on contact", () => {
    const state = startGame();
    clearStage(state);
    state.players[0].equipment.weapon = weapon(50, "test_hammer", 3); // battered
    state.items = [{ id: 1, kind: "repair", pos: { ...state.players[0].pos } }];
    step(state, idle, DT);
    ...
    expect(state.players[0].repairKits).toBe(1);
    expect(state.players[0].equipment.weapon.durability).toBe(3);
  ...
  it("stack up to the cap, then overflow stays on the ground", () => {
    const state = startGame();
    clearStage(state);
    state.players[0].repairKits = CONSUMABLES.stackCap; // full
    state.items = [{ id: 1, kind: "repair", pos: { ...state.players[0].pos } }];
    step(state, idle, DT);
    // No room: the kit stays grounded and the stack holds at the cap.
    expect(state.items).toHaveLength(1);
    expect(state.players[0].repairKits).toBe(CONSUMABLES.stackCap);
  });
});
...
describe("repair kits — spending", () => {
  it("mend the held weapon AND every weapon in the bag, not just the equipped one", () => {
    ...
    state.players[0].equipment.weapon = weapon(50, "test_hammer", 3); // battered
    state.players[0].inventory[0] = weapon(51, "test_pistol", 2); // battered spare
    state.players[0].inventory[1] = weapon(52, "test_wand", 0); // broken spare
    step(state, useRepair, DT);
    expect(state.players[0].repairKits).toBe(0);
    // The held weapon is whole again…
    expect(state.players[0].equipment.weapon.durability).toBe(
      weaponDef("test_hammer").durability,
    );
    // …and so is every weapon riding in the bag (the broken wand woke up).
    const pistol = state.players[0].inventory.find((i) => i?.id === 51);
    const wand = state.players[0].inventory.find((i) => i?.id === 52);
  ...
  it("re-equip the weapons durability booted, earliest-shed reclaiming the hand", () => {
    ...
    expect(state.players[0].equipment.weapon.unequippedAt).toBeUndefined();
