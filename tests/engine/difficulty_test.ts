...
describe("the drop economy (medkit/powerup mults)", () => {
  ...
  it("shrinks the medkit slice on the harder rungs", () => {
    ...
    // ladder]. 0.42 sits inside MEDIUM's medkit window but past JESUS's shrunken
    // one (the powerup slice ahead of it shrinks too, which narrows the window
    // from both sides), so on JESUS it falls through to the repair slice. Both
    // heroes have empty pouches, so the APPETITE gate leaves the slices whole.
    const medium = killForLoot("medium", [0.9, 0.9, 0.9, 0.0, 0.9, 0.42]);
    expect(medium.some((i) => i.kind === "medkit")).toBe(true);
    const jesus = killForLoot("jesus", [0.9, 0.9, 0.9, 0.0, 0.9, 0.42]);
