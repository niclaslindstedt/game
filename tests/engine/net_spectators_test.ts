...
//
// The first networked milestone is deliberately "eight machines, one hero": it puts real
...
describe("seating", () => {
  ...
  it("never sends a spectator anybody's bag", () => {
    ...
    // manipulate it, which is what makes the trade window honest.
    const { live } = session();
  ...
  it("seats a leaver's slot again rather than doubling up on the last one", () => {
    // `clients.size` was the obvious answer and it is wrong the moment anybody
    // leaves: two clients would share a seat, and two heroes would
...
describe("what a spectator may do", () => {
  it("may talk", () => {
    // The whole reason chat shipped with the wire itself rather than with the
    // rest of the party features: eight people
