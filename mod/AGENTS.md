...
| Pixel art, as a character grid                               | `sprites/<family>/<name>.yaml`                                                           | [`../content/sprites/`](../content/sprites)                                      |
| Pixel art, DRAWN (a real PNG)                                | `sprites/<family>/<name>.png`                                                            | [`FORMAT.md`](FORMAT.md#spritesfamilynamepng--pixel-art-you-drew-somewhere-else) |
| How that art MOVES (a walk past two frames, a talking mouth) | `animations.yaml`                                                                        | [`FORMAT.md`](FORMAT.md#animationsyaml--how-your-art-moves)                      |
...
| `no sprite "x"` (from `animations.yaml`)                        | A clip names a frame nothing draws. This one fails SILENTLY at play time if it gets through — the body flickers out of existence one frame in six.                |
| `"x" is not a state the game plays`                             | A clip's state must be one of `idle`, `walk`, `talk`, `cast` — the four moments the renderer actually asks about.                                                 |
| `the first bytes are not a PNG`                                 | A file in `sprites/` was renamed rather than exported. Read from the bytes, not the name.                                                                         |
| `interlaced (Adam7)` / `16 bits per channel`                    | Re-export the PNG without interlacing / at 8 bits. Both are non-default options in every editor.                                                                  |
