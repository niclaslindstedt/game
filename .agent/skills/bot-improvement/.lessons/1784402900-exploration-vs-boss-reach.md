...
- **A coverage-only gate deadlocks combat-bogged seeds.** If the rule is "explore
  ...
  the boss* — it loops on fog for the whole clock. Measured on `goodco_hq` easy:
  seeds that WON at baseline turned into timeouts that never even reached PROTOTYPE.
...
Verify with `simulate-run --max-minutes 8` across seeds 1-5 and read the "PROTOTYPE
