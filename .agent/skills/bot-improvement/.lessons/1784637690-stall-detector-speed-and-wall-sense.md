...
- **The oscillation cure is a committed wall-end query, not a better fan.**
  ...
  the blocked one, first body-width-open sweep per side wins) and LATCH the
  chosen side on the bot (`bot.trace`) while the straight sweep stays
  blocked, passing it back as `preferSide` so the trace never flips ends
  mid-wall. Clear the latch the moment a straight sweep runs clear. Two
  soundness rules learned by test failure: a bearing only counts as seeing
  the end when its open sweep reaches PAST where the blocker stands (a
...
- **The sim now watches through a camera by default.** `simulate-run` stamps
  ...
  screen — so absolute kill counts drop vs old reports (~-19% measured on
  the easy campaign). Never A/B a camera run against a pre-camera baseline
  without noting this.
