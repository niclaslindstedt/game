...
| 3b | Game — roster | `content/enemies/<biome>/<id>.yaml` | Elite/boss `dialogue` + `lastWords`; if a mob is **added, removed, or replaced**, its YAML (hp/damage/role/mechanics) and any `shieldedBy`/`flees`/`spareable`/`apparition` wiring. Load the `enemy-design` skill for the numbers. |
