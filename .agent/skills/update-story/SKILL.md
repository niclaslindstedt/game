...
| 3d | Game — thoughts | `content/thoughts.yaml` | The hero's pinned beats (kill/sighting/scripted-strike pins from a `LevelDef`, and a **travel door's `unready:`** — what he says at a door with no open road, which REPLAYS rather than firing once). A beat may be an **exchange**: `voice: { speaker, portrait }` names who answers him and a `{ them: [...] }` page is theirs — the mirror of `{ hero: [...] }` in an arrival scene. Reach for this, NOT `EnemyDef.dialogue`, when a line has to land inside a scripted beat. |
