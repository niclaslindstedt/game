...
| Sprites | one YAML per frame in `content/sprites/<family>/` — frames named exactly `<sprite>_0`/`<sprite>_1`. The house shape is minion 16×16, elite 24×24, boss 48×48, but nothing enforces it and the shipped roster deviates freely (minions run 16×16 to 24×24; three bosses are 24×24). Size the art to the mob's PRESENCE, and check it against its neighbours in the family sheet |
