...
- **Judge a candidate by the XP SHARE IT PAYS, never by an A/B of two
  ...
  `a / (1 + a)`. That read has a fraction of the variance and is what the
  shipped numbers were set against.

**`loot.intendedLevelByDifficulty` is each map's PACING YARDSTICK** — the level
a normal run ends at, authored in `content/levels/<id>.yaml`. The campaign
simulator's realistic pacing and its boss-level verdicts read it, and the
autopilot reads it to decide whether a scroll is still worth a detour. It is a
yardstick and never a faucet's gate: no payout is priced off it. When the curve
moves, re-read it off `--by-level` and update the level YAMLs.
...
| Knob | Does |
| --- | --- |
| `tierLevelCostStep` | **Per-difficulty slowdown.** Each difficulty TIER above the three bottom lanes makes a level cost this fraction MORE, COMPOUNDING per tier (`(1+step)^tier`, tier = `difficultyDef.index−3`): at the shipped `0.625`, nightmare ×1.625 and jesus ×2.64. So harder rungs take "longer and longer" to level — the steep step is what holds the nightmare full-clear landing at ~55 against the cheap mid-curve. Applied in `xpToLevelUp(level, difficulty)`; runtime-scalable via BALANCE › LEVEL SLOWDOWN. 0 = every difficulty alike. |
| `endgameSteepenFrom` / `endgameSteepenRate` | **Endgame wall — SHIPPED OFF.** The wall now lives in `content/leveling.yaml` itself (kills-per-level climbs from level 70 to ~1000 at 98), so `endgameSteepenRate` ships at **0** and `endgameSteepenFrom` (70) does nothing until a non-zero rate is restored. Restore one and every level past the threshold costs an extra `rate` COMPOUNDING on top of the authored table, for EVERY difficulty; BALANCE › ENDGAME WALL scales it. |
...
2. **Model it** with the calculator — no game needed:
   ...
   per level: `xpToNext`, `kills/lvl` (kill XP only), `w/scrolls` (the same with
   the XP-scroll faucet's duty cycle folded in), `levels/day`, and cumulative kills/days to
   the cap. `levels/day` and the cumulative columns ride the `w/scrolls` count —
   the realistic pace — while `kills/lvl` stays as the kill-XP-only baseline; the
   gap between them IS the scrolls' contribution (`--luck N` grows it, JESUS
   collapses it to zero). `--campaign` instead simulates clearing every level along
   the CRITICAL PATH (scrolls included, and the per-map XP caps applied) and
...
3. **Measure the real pace and GRAPH it** — the calculator's kills/hour is an
   ...
   node scripts/simulate-run.mjs --no-xp-scroll # the pure kill grind (isolate the scroll levers)
   ```
   ...
   node scripts/leveling-pace.mjs --run --no-xp-scroll
   ```
   ...
   `content/leveling.yaml` and `engine/game/config/leveling.ts`, so commit the
   change there and re-verify at `1×`.
