...
| `bossHpFrac` | Wound the level's OWN boss (and any escorting boss beside it) to a fraction of its bar — the one body a scenario cannot place, because it is the level's. Never kills: a sliver stands, so what follows is a real kill down the ordinary path (the death FX, the corpse, the drops). Two things read the fraction: the renderer's wound sprites (a boss's dying last stand at ≤0.1) and `EnemyDef.phases`, so `0.3` stages a two-phase fight ALREADY IN ITS SECOND PHASE |
...
them, pruning the stale, merging the duplicated, and promoting the proven ones
into the recipes above (`node scripts/skill-lessons.mjs test-scenario --list`).
