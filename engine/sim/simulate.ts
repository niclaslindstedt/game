...
// game-spec:allow-large-file: the headless campaign simulator — one cohesive measurement harness and report model
...
//
// Deliberately NOT exported from engine/index.ts: the app never simulates —
...
function playRun(args: {
  ...
  simulation: while (now() < maxTimeMs) {
    ...
    // …and the ROOM half ahead of them (bot/intent.ts `runBotErrands`): work
    ...
    runBotErrands(bot, state, state.players[0]);
    ...
    if (bots.length > 1) {
      ...
      for (let seat = 0; seat < partyInput.length; seat++) {
        ...
        // and call it balance.
        runBotErrands(seatBot, state, hero);
