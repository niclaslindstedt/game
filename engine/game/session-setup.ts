...
export type RunParams = {
   ...
   *
   * The trip back from GOODCO is played (engine/game/drive/), so the hero pulls
   ...
   * pays for THE SCALE, one rung deeper each difficulty (engine/game/cache.ts).
   ...
   * — read only by a venue with a `sky` (`engine/game/daylight.ts`).
   *
