...
// game-spec:allow-large-file: run setup — one cohesive, order-sensitive world-generation sequence
...
export function createGame(
  ...
  // not paid yet (engine/game/cache.ts). A session parameter, seeded from the
  ...
  // ship (see engine/game/vehicles.ts). The machines themselves are drawn
...
export function createHero(
  ...
  return {
    ...
    vault: [],
    // THE CACHE's cells (engine/game/cache.ts) — always laid out, even for a hero
