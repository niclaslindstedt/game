...
// everywhere:
//   - the AUTOPILOT (engine/game/bot/index.ts `botAllocate`) spends the live hero's
//     points against the build rotation, tick by tick, in the real-loop
//     simulator (engine/sim/simulate.ts) and the app's `?bot=` autoplay;
//   - the ANALYTIC paper sim (engine/sim/analytic.ts) spends them via
