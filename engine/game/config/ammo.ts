...
export const AMMO = {
  ...
  stackCap: 200,
  /**
   * THE OPENING HOLSTER: how many rounds a run starts with, of the kind the
   * DIFFICULTY'S OWN starting weapon eats — and of no other kind at all. A
   * hundred shots is a long opening — over a minute of steady fire — which is
   * the point: the first thing a new player learns about ammunition should be
   * that it exists, not that it is about to run out. A melee or magic opening
   * stocks nothing, because there is no longer a fallback gun behind the hero
   * to stock for (see `UNARMED_DEF_ID`).
   */
  starting: 100,
  /**
