...
export type GameEvent =
  ...
  | {
       ...
       * `engine/sim/aoe-calibration.ts`): "at this effective `arc`, how many
  ...
  | {
       ...
       * `engine/sim/aoe-calibration.ts`). Absent on melee, ability, and companion
       * blows. */
      fromVolley?: number;
    }
  | {
   ...
   * garage's north wall (`grantCache`, engine/game/cache.ts). Fires once, on the
   ...
   * rolling DPS/kill-rate output) — so the balance instruments (engine/sim) can
