...
export type GateState = {
  ...
  entered: boolean;
  /**
   * A SEAM ONE HERO TORE HOME (`engine/game/rift-tool.ts`), rather than a door
 ...
 * (engine/game/vehicles.ts) — deterministic clockwork, no rng.
 */
type VehicleBase = {
...
export type CarVehicle = VehicleBase & {
   ...
   * moves the BODY across (`applyCarWheel`, engine/game/vehicles.ts — the road's
 ...
 * `engine/game/arrivals.ts`). The five run strictly in order and every one of
 ...
 */
/**
 * ONE SIDE OF AN OPEN TRADE (`engine/game/trade.ts`).
 *
