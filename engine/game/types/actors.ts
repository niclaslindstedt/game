...
export type Player = {
   ...
   * split's other half** (`engine/game/bot/intent.ts`). Everything else the autopilot
  ...
  vault: Equipment[];
  /**
   * THE CACHE (engine/game/cache.ts): the antique chest against the garage's north
...
export type Enemy = {
   ...
   * or `phases` — see engine/game/mechanics.ts; absent on everything else).
