...
- **Boss ability catalog — THE FLAGBEARER's laser eyes and planted flag** — Bosses now carry named abilities from a catalog instead of permuting the same
  four moves: THE FLAGBEARER opens his eyes and sweeps a beam that leaves the regolith
...
- **Every boss gets a signature move** — PAYLOAD-1 fires a fan of coins that ricochet off the walls and salts the floor with
  bait that looks exactly like loot; THE FOUNDER stops summoning and starts
  SHIPPING, calling drop pods down on marks around you that burst and pop open —
  and on nightmare, calling his followers in at a dead run. THE BRO SUPERCORE
  dispatches units to your coordinates, BRO OMEGA and THE VAULT WARDEN sweep the
  floor with a beam, and the The Founder you meet in the rift no longer wears the same
  clothes he wore on Mars.
- **The last two boss moves, and a cast pose on every boss** — THE BRO SUPERCORE now restores itself from a repair node you can see and break,
...
- **Repair kits** — A repair-kit pickup restores the equipped weapon's durability to full; with nothing to repair it stays on the ground for later. THE FLAGBEARER always drops one.
...
- **LEVEL 1 - GOODCO HQ** — The story's first level: raid GOODCO headquarters for the interplanetary
...
- **Unique story mobs, dialogue scenes, and locked rooms** — Every level now fields 3-4 unique named mobs that rush into view and reveal the plot in tap-through dialogue scenes (bosses got longer stare-down scenes of their own); killing one drops a signature weapon plus story items — keycards that open locked rooms and documents that unravel the GOODCO moon conspiracy.
...
- **The space suit is loot now** — At GOODCO HQ the hero starts in his living-room clothes and only becomes the astronaut once he loots the EVA SPACE SUIT — an epic drop from the Chief of Security — which both armors him and swaps his sprite for the rest of the game.
...
- **SUCCESSOR robots on GOODCO HQ** — GOODCO HQ now fields SUCCESSOR units — humanoid robots that replace some of the human night shift: tougher, harder-hitting regular monsters (they throw sparks, not blood, when struck) that reward the extra effort with a much richer drop roll than the staff around them.
...
- **Rocks, craters, and SUCCESSOR on the moon** — The moon now scatters solid moonrock slabs (that block sight, shots, and a nuke's blast) and jumpable craters you can leap but not cross, and GOODCO's SUCCESSOR robots garrison it — with the hero's own inner monologue the first time you take one down.
...
- **The AI CORE room** — THE ARCHITECT now drops a CORE KEYCARD that unlocks a new locked AI CORE room in GOODCO HQ, where the superintelligence he built keeps its logs.
...
- **"Level 3: MARS"** — The campaign continues to Mars: a secret GOODCO colony where rovers work the red dust outside and robots and fembots staff the base inside — three tech-magnate elites (THE INDEXER, THE VENDOR, THE SEED), a locked lizard shrine, a new desert-western theme ("RED DUST"), and THE FOUNDER, the first boss who doesn't die: he cowers, reveals what Ada was traded for, and flees through a rift. THE FLAGBEARER's moon ending now points the way (the moon was GOODCO's disastrous mistake), and the colony is staffed by SUCCESSOR units.
...
- **More inner monologues** — Two new first-kill inner monologues: the hero's surprise at a fully staffed GOODCO HQ at midnight ("good thing I brought the sword") on the first intern kill, and his arrival read on the moon's haunting on the first wisp kill.
- **SUCCESSOR sight monologue** — Seeing the first SUCCESSOR at GOODCO HQ now plays a new inner monologue: the hero helped build the first unit before the AI redrew the line and it took everyone's jobs — now the tables turn.
- **SUCCESSOR PRIME** — Mars gains a fourth elite: SUCCESSOR PRIME, the robot foreman orchestrating the SUCCESSOR line — the future of agent orchestration, coming for the AI engineers' jobs too. Drops the PROMPT INJECTOR and the ORG CHART.
- **The sword draws on the first strike** — GOODCO HQ now opens with the hero's sword holstered — the auto-attack sits out until a lone scientist breaks from the pack and takes a harmless first swing at him, which draws the blade ("good thing I brought the sword") and turns combat on.
- **The wall arsenal — a starting weapon per difficulty** — The weapon on the hero's living-room wall now depends on the chosen difficulty — THE LICENSED REPLICA (easy), MEDIEVAL SWORD (medium), COMBAT KNIFE (hard), BRASS KNUCKLES (nightmare), A STICK (JESUS CHRIST!) — and the prelude cutscene always shows the exact piece the run starts with.
...
- **World-drop relics (boss-run loot)** — Five level-locked UNIQUE relics that drop out in the world — not from one fixed
  ...
  runs once EASY is beaten: THE FIRST DRAFT (GOODCO HQ), THE PALE COVENANT (the
...
- **Uniques for every build on easy and medium** — Four new world-drop uniques round out build coverage: DEADSTAR (Moon, easy magic), PALE RIDER (Boot Hill, easy ranged), and Boot Hill's two medium melee relics HERDBREAKER and THE LAST ROUNDUP — Boot Hill now has farmable uniques of its own.
- **THE LEAK** — Boot Hill fields a fourth elite: THE LEAK, the whistleblower in exile whose leaked archive is what the BRO SUPERCORE was trained on — the game's first ranged elite, fighting from behind cover and dropping the DEAD MAN'S SWITCH charm and THE SNOW ARCHIVE.
...
- **Travel cutscenes between levels** — Every level now opens on a travel cutscene: the garage launch and the flight to the moon, THE FLAGBEARER's send-off and the crossing to Mars, walking into the rift THE FOUNDER tore open, and stepping out of the far door into Boot Hill's daylight.
...
- **Ada's Trail & the bunker's prison twist** — Ada now leaves an escalating found-lore trace on every campaign level (Ada's Trail), the secret bunker is revealed as a prison the CORE has already emptied — its guards are wardens and THE SAFETY OFFICER knows but is too afraid to say — and the bunker key (RASPUTIN's SEVERED HAND) now drops only after BOOT HILL is cleared.
...
- **"Level 5: BOOT HILL"** — The campaign finale — the rift's far side is BOOT HILL, a knockoff wild-west theme park built by The Strongman and The Stunt Double and run on TRUST ME BRO robotics: a tight frontier town of building-sized houses, robot cowboy hands, named elites (The Stunt Double, The Strongman and his sellable brand watches, The Leading Man), The Founder finally dying (dropping the new zero-stat TRASH tier), a merchant stall that fences The Strongman's estate at unique odds, and THE BRO SUPERCORE boss — shielded by three cover-taking BRO controllers that shoot back — whose death quakes the park and plays the campaign's black-screen epilogue.
...
- **Spinning hay balls in BOOT HILL** — BOOT HILL's main street now has rolling hay balls: golden bales bounce and spin in from the east, and a bale that catches the hero costs a very slight bit of health and shoves him back down the street — step out of its lane or jump it to shake it off.
...
- **Stampede approach rumble** — The GOODCO HQ employee stampede is now heard before it is seen — a low rumble of feet fades up over the last moment of the countdown and swells as the wall charges in, peaking as it passes, so the herd never arrives on a silent floor.
- **Stampede approach-dust telegraph** — A GOODCO HQ employee stampede is now SEEN coming as well as heard: a line of dust kicks up along the exact lane the wall will charge down a beat before the runners appear, so you know which band to clear. The warning is longer on the gentle difficulties and a mere blink on the hardest (1.5× the lead on easy down to 0.4× on JESUS).
...
- **Two new powerups every map** — Every map past GOODCO HQ now introduces TWO new powerups and keeps everything
  ...
  rift, DEAD MAN'S HAND and IRON STAMPEDE in Boot Hill, and CONTINUITY PROTOCOL
...
- **Moon reforged into ridges and basins** — THE MOON is rebuilt around discovery: three long rock ridges with offset pass-gaps carve the open regolith into basins the hero weaves through on an authored path from the lander to THE FLAGBEARER, the finite ridge-gap spawners ramp wisps → ghosts → wraiths → SUCCESSOR, and two off-path detour pockets (the CRASHED LANDER and THE THIRTEENTH GRAVE) each hide a chest guarded by a pinned rare/unique, with a STILL POINT merchant nook before the boss.
...
- **The game is now "Ada's Trail"** — The game has a name and a story-first identity: Ada's Trail — survive the search for your lost love. Title screen, PWA manifest, and social previews all follow.
...
- **GOODCO monologue on sight** — The hero's GOODCO HQ inner monologue about the fully staffed night shift now plays the moment he first sees an intern, instead of after his first intern kill.
...
- **Welfare-era intro beat** — The GOODCO HQ opening monologue now notes the hero and Ada are on welfare like everyone else replaced by AI — but at least there's Webflix.
- **Snappier GOODCO HQ opening** — Reworked the GOODCO HQ opening for flow and pace: a tighter, less repetitive intro monologue and first inner monologue, and the movie-night prelude now sits the couple side by side on the sofa watching the TV before Ada heads out.
...
- **GOODCO HQ art pass** — Redrew the six weakest-reading GOODCO HQ sprites: SUCCESSOR (a bigger heavy robot), THE JANITOR (mop and bucket), THE ARCHITECT (a legible half-machine), the office desk, the vending machine, and the HAZMAT TECH (a bulky suited tank).
...
- **GOODCO HQ wall & door art** — Redrew GOODCO HQ's wall and locked-door tiles: the flat wall block is now a recessed steel lab panel with a bevel and corner bolts, and the locked door is a sealed blast door with a bold red maglock.
- **Boot Hill art pass** — Redrew the five weakest-reading Boot Hill sprites: the cowbot, saloon brawler and tin outlaw hands now read as distinct robot cowboys (greeter, bruiser, masked gunslinger), the longhorn is a looming iron-steer heavy on a bigger canvas, and the park gate is a legible signed BOOT HILL billboard instead of a plank table.
...
- **Clearer item & weapon icons** — Redrew three weak-reading inventory icons: The Stunt Double's ponytail (now a tied,
...
- **GOODCO HQ onboarding pass** — The first level now builds and releases: a gentle opening tempo, a mid-floor
...
- **GOODCO HQ grocery-store aisles + detour lockers** — The first level's sales floor is now laid out like a grocery store — a
  ...
  BREAK ROOM, the latter guarded by the EMPLOYEE OF THE MONTH. A new GOODCO
...
- **Finite spawn points replace the endless wave** — A level's horde can now be authored as SPAWN POINTS — placed points that arm as
  ...
  walls (line-of-sight gated). GOODCO HQ is rebuilt on this model.
...
- **Every campaign map is a finite, clearable horde** — Moon, Mars, Boot Hill, and The Rift are converted from the endless wave stream to
  placed SPAWN POINTS (like GOODCO HQ) — each map's horde is now a finite count the
...
- **The Moon rebalanced** — THE MOON's difficulty now ramps evenly from the lander to the flag — the horde starts gentle and grows tougher toward THE FLAGBEARER — and the spawn knots are thinned so a level-appropriate hero clears them on the move instead of getting pinned.
- **Moon reachable, GOODCO leveling** — Retuned THE MOON so its boss is actually reachable — thinned the finite spawn knots, dropped the con ramp and THE FLAGBEARER's engage level onto the hero's real arrival band, pulled the tanky SUCCESSOR out of the mid basins, and moved the boss surge onto the approach so the climax is winnable instead of a wall; and widened GOODCO HQ's spawn caps so an engaged player levels up before MUSKRAT.
...
- **Mars redesign** — MARS is rebuilt as a two-act desert→colony assault: an explicit serpentine
  ...
  keycard detour. THE FOUNDER now flees into the rift at a quarter health instead
...
- **Boot Hill rebuilt as a Main Street town gauntlet** — BOOT HILL is redesigned into a claustrophobic frontier-town gauntlet: a new cast
...
- **Slower employee stampedes** — Halved the GOODCO HQ employee stampede's charge speed, giving the hero more time to read the incoming herd and hop or step out of its lane.
- **GOODCO HQ is a spaceship assembly line, bossed by PAYLOAD-1** — Level 1 is rebuilt as a working spaceship factory — three readable floor zones
  (offices, assembly deck, launch bay), conveyor assembly lines running down each
  build bay, welder-bot ASSEMBLER mobs, and a memecoin-robot boss, PAYLOAD-1,
  replacing MUSKRAT. Payload is seeded here as the machine that keeps one man rich and
...
- **GOODCO stampedes are rarer and easier to hop** — The GOODCO HQ employee stampede now charges in far less often (roughly a third as frequently), the runners are half their old size, and the herd's collision band is a thin vertical line — so a well-timed jump clears the wall cleanly instead of the old wide slab that was nearly impossible to hop.
- **Stampedes knock minions out instead of killing them** — A GOODCO HQ employee stampede now BOWLS the minions in its lane over — flinging them aside and knocking them out for a few seconds — instead of killing them outright, so a herd no longer thins the horde as it passes (and still can't be farmed).
- **A gentler early game** — The first level eases up so a new player almost never dies learning the ropes:
  ...
  more blows), the GOODCO HQ opening knot is capped lower so the starter weapon can
  ...
  hold off until you are halfway to PAYLOAD-1, and the SECURITY BATON hits a touch
  harder.
- **"GOODCO HQ: a gentler onboarding"** — Rebalanced the opening level so the back half past the NIGHT MANAGER is no longer a wall. EASY now opens on a FIRE EXTINGUISHER — a short-range cone spray that clears a whole pack — instead of the single-target wand, and every rung's starter holds far longer before it wears out (so a fresh player is never stranded on the feeble sidearm mid-floor). The scripted drops carry the run: the SECURITY BATON hits harder, a 9MM PISTOL and a repair kit are handed over for the back half, and the NIGHT MANAGER's EXECUTIVE PUTTER is now a real single-target blade (was a downgrade that auto-equipped over your weapon) with PAYLOAD-1's PLASMA CUTTER trophy kept clearly above it. The assembly floor's tougher staff — engineers, assemblers, guards, hazmat techs and SUCCESSOR units — all pack less health.
...
- **GOODCO HQ working floor** — The GOODCO assembly floor now feels staffed and alive: the conveyor belts roll
  ...
  intern, a guard, and roaming SUCCESSOR units sweeping the bays), and the whole
...
- **The bestiary opens on the critters, not on the bosses** — The library's bestiary index and its front-door rack now list each venue's rank
  ...
  alone — so the three THE FOUNDERs read as MARS, THE RIFT and BOOT HILL in a rack
...
- The bunker's database emperor now goes by his real fake name: THE ROOT
  (formerly JERRY).
...
- **Autopilot holds its ground for the opening strike** — The autopilot no longer flees the scripted "draw your weapon" opening at GOODCO HQ. While holstered the hero used to kite the lone vanguard, backpedalling the whole pack across the floor into the far wall for several seconds before the harmless first swing ever landed; he now stands his ground once he's at the standoff and takes the scripted hit, arming roughly three times faster.
...
- **Inner-monologue portrait matches the hero** — The hero's inner-monologue portrait now shows whatever he's actually wearing, so his GOODCO HQ thoughts no longer flash the EVA-suit portrait before he's found the suit.
- **The opening scientist stops at the hero** — The lone GOODCO HQ scientist that rushes the disarmed hero no longer clips into him and shoves him around — it now stops the instant it's next to him to land its harmless first swing, then folds back into the pack at normal mob speed once the blade is drawn.
...
- **GOODCO HQ opening beat can't be stalled** — The scripted vanguard at GOODCO HQ now draws the hero's blade and fires the opening dialogue when it closes to within range, rather than only on contact — so circling the rusher can no longer stall the intro indefinitely.
...
- **GOODCO HQ opening strike lands on contact** — The vanguard scientist now reaches the hero before its first swing draws his
...
- **Party no longer strands the hero disarmed** — At GOODCO HQ the hero draws his blade even when his companions (or a conjured power) cut down the scripted vanguard before it reaches him — previously that left the hero disarmed for the whole level, unable to attack while his party fought on.
...
- **BOT VIEW plays the scripted opening and pauses on app-switch** — DEVELOPER → BOT VIEW no longer soft-locks a level with a scripted opening strike (GOODCO HQ): a leveled arrival hero whose difficulty read-ledger already holds the "draw your weapon" beat now still arms when the vanguard reaches him, instead of standing holstered while the pack piles up around him. Switching away from a watched BOT VIEW run now pauses it (and keeps it paused) instead of playing on in the background.
...
- **Demo starts GOODCO with the difficulty's start loot** — The HOW TO PLAY demo (and developer BOT VIEW) no longer opens the first mission with a leveled kit of randomly rolled gear — on the campaign's opening map a starting lane now begins from a fresh rookie's difficulty start loot, exactly as a new player would.
