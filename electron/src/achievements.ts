...
// Identical in shape to native/src/achievements.ts, and deliberately so.
// ACHIEVEMENTS' SHELL half (desktop) — the bridge between the game's badge ledger
...
export function createAchievementsBridge(
  ...
  const report = async (
    ...
    // straight into the platform, and an unmapped id would be reported as a badge
