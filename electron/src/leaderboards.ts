...
// Identical in shape to native/src/leaderboards.ts. It is wired up even though
// `leaderboardsProvider()` is null on this shell today (see the seam for why),
// so a future provider needs no bridge work.
// LEADERBOARDS' SHELL half (desktop) — the bridge between the game's board catalog
...
// own ids (Play generates them; Steam and Game Center let us choose ours). An
 ...
 * WebView (main.ts wraps `executeJavaScript`); `handle` takes each parsed
...
export function createScoresBridge(
  ...
  const submit = async (
    ...
    // Int64, so a NaN or a fraction would go straight into the platform, and an
