...
//
// VITE_DEV_TOOLS follows the PROFILE: the `production` profile — the build
// uploaded to Steam — builds with it off, which strips the hidden sun-tap
...
// `index.html` (`stripBootShell` in pwa/pwa-plugin.ts). Nothing crawls an asar,
// JavaScript is never off in here, and the bundle is on local disk — so all
// that markup did was flash an SEO document between the window opening and the
// game's own studio card.
//
