...
- **A fresh clone can't simulate.** `engine/generated/` (levels, enemies,
  botTuning) is gitignored and only exists after `npm run levels` / `make
  assets`. A baseline batch launched before that dies with
  `ERR_MODULE_NOT_FOUND: engine/generated/botTuning.ts` and writes NO `--json`
...
- **Baseline means the ORIGINAL code.** Once edits are in the working tree,
  capture the before-side with `git stash push engine/game/bot.ts ...` → run the
