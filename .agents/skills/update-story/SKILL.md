...
docs/manuscript.md (TIER 2 — the script: every line, verbatim)
      │  extrapolated into
content/**           (TIER 3 — the game: roster, items, cutscenes)
```
...
`.agents/skills/update-story/.last-updated` holds the git commit hash from the
...
1. Read the baseline:
   ...
   ```sh
   BASELINE=$(cat .agents/skills/update-story/.last-updated)
   ```
...
3. Decide the direction of the pass:
   ...
   - **A drift/reconcile sweep** (the data or manuscript moved under the story
     without a matching top-tier edit — e.g. a `content/**` diff touched
...
| 3a | Game — cutscenes & monologues | `content/cutscenes/<id>.yaml`; `content/levels/<id>.yaml` (`intro`, `outro`, `merchant.greeting`, `firstKillThoughts`/`firstSightThoughts` pins) | Prelude + travel scenes, per-level opening/closing monologues, merchant lines, thought pins. |
| 3b | Game — roster | `content/enemies/<biome>/<id>.yaml` | Elite/boss `dialogue` + `lastWords` + `mechanics.abilities[].bark` — and the two that carry the HERO's voice rather than the mob's: a `- hero:` page inside `dialogue` (his half of the scene) and `deathBark` (his line as a boss falls). If a mob is **added, removed, or replaced**, its YAML (hp/damage/role/mechanics) and any `shieldedBy`/`flees`/`spareable`/`apparition` wiring. Load the `enemy-design` skill for the numbers. |
...
| 3g | The LIBRARY — where a reader meets the line | `pwa/scripts/library/model-story.mjs` (`thoughtsOn`), `render-story.mjs` (the heading map), `model-missions.mjs` (`LEVEL_FIELDS`) | Only when the beat is fired by a trigger the library does not already name. `thoughtsOn()` builds a chapter from a HARDCODED list of trigger sources, so a beat fired by anything else is authored, shipped, playable and on no page at all — with every test green. Verify by grepping the built pages (below), never by reading the model. |
...
- [ ] Write the new baseline:
      ...
      git rev-parse HEAD > .agents/skills/update-story/.last-updated
