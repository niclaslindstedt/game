...
// (`content/item_quality.yaml`, `content/item_rarity.yaml`) into the engine's
...
// Source of truth: content/items/<rarity>/<id>.yaml + content/item_quality.yaml
// + content/item_rarity.yaml. Regenerate with \`npm run levels\` (also runs
// inside \`npm run assets\` / \`make assets\`).
// SPDX-License-Identifier: PolyForm-Noncommercial-1.0.0`;

const json = (v) => JSON.stringify(v, null, 2);

/** A base cooked for the SHIPPED catalogs: the shared cook, minus the reader-
 * facing `description` (see the lore module's note below). */
const shippedDef = (doc) => {
  const def = baseDef(doc);
...
/** The make-quality axis (content/item_quality.yaml) — read by config
...
/** The tier ladder + rarity economy (content/item_rarity.yaml) — read by the
...
// looks at it. Left on the def it is 9 KB gzipped of English prose sitting in
// the app's STARTUP chunk — the menus need the bases (names, icons, level
// requirements) and tree-shaking is global, so the prose came with them. This
// is the only field on a base that no rule reads, which is exactly why it is
// the one that can be lifted out.
//
// NOT stripped in `compile.mjs`: that module is shared with the MOD compiler,
// and a mod's defs reach the engine through `registerDefs` with no second
// module to carry their lore. A shipped base simply has no `description` at
// runtime; a mod's does. The field stays optional on the type for that reason.
const loreOut = `${banner}

/** Every hand-authored base's lore paragraph, by item id — read by the LIBRARY
 * generator (pwa/scripts/library/) and by nothing that ships. */
export const GENERATED_ITEM_LORE: Record<string, string> = ${json(
  Object.fromEntries(
    [...weapons, ...gear]
      .filter((e) => e.doc.description)
      .map((e) => [e.id, e.doc.description]),
  ),
)};
`;
