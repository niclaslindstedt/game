...
//      menu-facing summaries, the campaign / secret / display order arrays and
//      the stamina ladders, read by engine/game/defs/levels/summary.ts).
...
// Campaign in story-index order; secret venues after, in file order; DISPLAY
// CASES last.
//
// A display case is compiled into the catalog like any level — `levelDef()`
// resolves it and `createGame` carves it — but it joins NEITHER order, which is
// the whole of what the flag buys. Everything outside a run walks
// LEVEL_ORDER/SECRET_LEVEL_ORDER (the campaign chain, the level picker, the dev
// warp rows, every page of the library), so a venue that exists only to be
// looked at is excluded from all of them by not being listed rather than by
// each of them learning to skip it.
const campaign = entries
  .filter((e) => e.campaign)
  .sort((a, b) => a.def.index - b.def.index);
const secret = entries.filter((e) => e.secret);
const display = entries.filter((e) => e.display);

const defs = [...campaign, ...secret, ...display].map((e) => e.def);
const campaignOrder = campaign.map((e) => e.id);
const secretOrder = secret.map((e) => e.id);
const displayOrder = display.map((e) => e.id);
...
/** The DISPLAY CASES — venues compiled into the catalog but in neither order
 * above (see \`display: true\` in the level YAML). Listed so the two orders can
 * still be checked against the whole catalog; nothing player-facing reads it. */
export const GENERATED_DISPLAY_ORDER: string[] = ${JSON.stringify(displayOrder)};
