...
// step further along. The mod compiler runs under the plain Node the desktop
// package carries, which has no TypeScript — and `scripts/asset-tools/script-schema.mjs`
...
// non-erasable syntax so `node` can read it in the repo), but the shipped
// runtime is whatever Node the packaging machine ran. A ship target resting on
// an experimental flag in a runtime somebody else upgrades is one that breaks
// in a released build, on a player's machine, for a reason nobody changed.
//
// The output lands in `modtools-lua/`, beside `server-dist/` — the other
// compiled-engine tree — and NOT under `mod/`. That is not
...
const outDir = path.join(root, "modtools-lua");
...
const configPath = path.join(root, "tsconfig.modtools-lua.json");
...
// nearest package.json above it says.
writeFileSync(
