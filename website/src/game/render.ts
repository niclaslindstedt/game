...
export function drawFrame(
  ...
  // the mob cull (below) and the fog draw (bottom): a mob is only drawn on
  // ground the hero has actually uncovered, never through the frontier stipple.
...
function ensureFogField(state: GameState): FogField {
 ...
 * hero can see.
 */
function fogDistanceAt(field: FogField, wx: number, wy: number): number {
  const cell = MAP.cellSize;
