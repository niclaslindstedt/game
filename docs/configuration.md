...
| Controls → Keys              | WASD move / mouse only (desktop-only row, follow-cursor mode) | WASD move on fine pointers (aim & shoot always walks by keyboard, so the row only shows in follow-cursor mode; hidden on touch)                                 |
