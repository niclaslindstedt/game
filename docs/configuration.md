...
| Gore → SFW mode                           | on / off                                                     | off (on replaces blood, gibs, cleaves, incineration and death-scene gore with pastel stardust and glitter. In the DRIVE, a struck body dissolves into wind-borne fairy dust and leaves nothing in the road, and every other collision leaves a smaller puff of it at the contact point — laid over the crash rather than instead of it: the damaged sprites, debris, smoke and shake all still play, and the marks a collision leaves on the road, the panels and the tyres are re-hued to the pastel palette while the wagon's own tread prints go down as dark rubber, a burning car and an exploding fuel tank throw gold stars instead of flame, and the crowd, THE GLUED and the bystanders all speak kid-friendly lines that match what is on screen. The detailed rows below are preserved and return unchanged when SFW is turned off) |
...
| Gore → Reset all                          | one action                                                   | turns SFW mode off and puts every kind of gore back the way the game shipped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
campaign earns. That screen carries two persisted knobs, and they sit in two
different places because they answer to two different things. **DIFFICULTY**
(`minigameDifficulty`, shipped at `medium`) is the shelf's — it picks the rung
out of the rungs the campaign has actually been beaten on, and a rung means the
same on every machine — so it sits under the cabinets, above BACK. A cabinet's
own way of being played sits **under that cabinet**, labelled out of its
definition and present only where the machine has a second way to offer: the
road's is **DIRECTION** (`minigameVariant`, shipped at `goodco_hq`) — out to
GOODCO or the same road home to the GARAGE. One pick is stored for the whole
shelf and each cabinet resolves it against its own list, so a machine that has
never heard of the stored id plays its own default rather than refusing to
start.
...
- **STANDING WALLS** — whether a `plane: wall` piece is EXTRUDED off its
  ...
  game did before the extrusion existed. **Off is the default and the shipped
  look**, because the shipped camera is square-on: the floor grid is still
  rectangles there and the flat panel already read as a wall, so a face hanging a
  hero's height off every panel is a perspective the rest of the picture is not
  drawn in. Turn it on with CAMERA YAW, where a flat panel stops reading as a
  wall at all and a lab's partitions become a slightly darker path across the
  floor. Unlike ANTI-ALIASING it is NOT folded together with the yaw — it means
  "I DO want the faces", so sweeping the camera back to square-on to compare the
  two looks leaves the answer where the developer put it.
...
The first lap opens on the LAUNCH cutscene, exactly as the arcade cabinet does —
`&launch=0` skips it, which is what a harness wants, since a held caption is a
`window.__flight` that never arrives. With `?debug`, the live `FlightState` is
exposed as `window.__flight`. Landing rolls straight into the next lap on a
fresh seed with the pre-flight card re-armed (and no cutscene — a lap is a
re-flight, not a fresh sitting); ESC leaves. Developer tooling, folded out of a
store build with the gallery.
