...
| Parameter            | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `?debug`             | Enables debug-level console output (`src/output.ts`, OSS_SPEC §19.3). All levels are always captured in the in-memory buffer regardless; the flag only controls console verbosity. Additionally exposes the live engine state as `window.__game`, the scenario hook as `window.__scenario(spec)`, and two animation-tuning hooks — `window.__swing({kind, weaponClass, t})` pins the field hero's held weapon at a fixed fraction `t` (0..1) of its swing arc (`null` clears it; for a melee swing, an optional `arc`/`range` also draws the matching slash pinned at the same fraction), `window.__timeScale(f)` scales the whole simulation clock so a fast animation runs at `f`× speed (`1` restores real time), and `window.__cast(spellId)` makes the hero unlock and afford the named spell, slots it, and fires it (for eyeballing the element-tinted cast FX — pair with `__scenario` for a target and `__timeScale` to slow it) — all in `website/src/game/GameScreen.tsx`, so DevTools, the playtest harness (`website/scripts/playtest.mjs`), the weapon-swing preview (`website/scripts/weapon-swing.mjs`), and the spell preview (`website/scripts/spell-preview.mjs`) can inspect and re-shape real runs. It also forces the in-run FPS meter on (the DEVELOPER menu's DEBUG MODE setting shows the same meter). |
...
| `?botProfile=<lane>` | The bot's combat PROFILE — the weapon lane its build commits to: `melee`, `ranged`, `magic`, or `auto` (default; the emergent lane from what it has invested in). Steers stat allocation into the lane's attributes (each lane still banks INTELLIGENCE for the reach/AoE-cleave/crit that helps every class), so the stat-aware auto-equip naturally fights that way. Only meaningful alongside `?bot=`. Unknown names fall back to `auto`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
