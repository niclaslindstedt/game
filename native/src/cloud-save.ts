...
// CLOUD SAVE's NATIVE half — the bridge between the game's sync engine
 ...
 * (App.tsx wraps `injectJavaScript`); `handle` takes each parsed cloud message
 * from `onMessage`.
 */
...
/** UTF-8 byte length of a string (no TextEncoder needed on Hermes). */
