---
type: Added
title: Host a game, join a friend's, and watch it together
---

The Steam build can now open a run to other players, and the title menu has the
three doors that reach it: **HOST GAME** (who may come in, how many seats, an
...
hidden), and **JOIN BY ADDRESS** (a typed `host:port`, with the addresses this
device has joined before one press away).

A joiner is a **spectator**: they see the run exactly as the host does, stand on
